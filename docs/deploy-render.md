...
- **Email** (reset links, invitations, digests, team messages): the blueprint sends through Resend (`VANTAGE_EMAIL_PROVIDER=resend` plus `RESEND_API_KEY`). To send from `vantageusmc.com` itself with no email service, switch to `VANTAGE_EMAIL_PROVIDER=direct` and finish it in **Owner console → Email**. See [email.md](email.md).
