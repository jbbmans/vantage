...
ctx.db.prepare('UPDATE users SET is_operator = 1, active = 1, password_hash = ?, must_change_password = 1, totp_enabled = 0, totp_secret = NULL, totp_pending = NULL, totp_last_step = NULL, updated_at = ? WHERE id = ?').run(hashPassword(password), new Date().toISOString(), user.id);
