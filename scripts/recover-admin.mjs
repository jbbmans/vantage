...
const { getDb, now, newId, audit } = await import('../server/db.js');
const db = getDb();
...
let target;
if (arg) {
  target = db.prepare('SELECT * FROM users WHERE username = ?').get(arg);
  ...
  if (admins.length !== 1) {
    console.error(`Found ${admins.length} administrator accounts — name one explicitly: npm run recover -- <username>`);
    process.exit(1);
  }
  [target] = admins;
}
