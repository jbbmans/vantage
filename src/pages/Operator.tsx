  ...
  return (
    <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
      <Panel title="GenAI.mil" subtitle={data.configured ? `key ${data.key_fingerprint} · ${data.base_url}` : 'no key configured'} action={<Button size="sm" variant="primary" onClick={save} loading={busy}><Save className="h-4 w-4" />Save</Button>}>
        {!data.configured && <p className="mb-3 rounded-md border border-warn/40 bg-warn/10 px-3 py-2 text-sm text-ink">Set VANTAGE_GENAI_API_KEY (and VANTAGE_GENAI_BASE_URL if not the default) in the server environment, then restart.</p>}
