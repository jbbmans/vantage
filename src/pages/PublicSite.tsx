...
export default function PublicSite() {
  ...
  return <div className="mission-site">
    ...
    {video && <section className="mission-container mission-video" id="watch"><div className="mission-section-heading"><div><p className="mission-eyebrow">INSIDE VANTAGE</p><h2>See the workflow.</h2></div><p>12 animated guides using sample data. Narrated, with optional captions.</p></div><div className="mission-video-layout"><video key={video.id} controls preload="none" poster={video.poster} aria-label={video.title}><source src={video.src} type="video/mp4" />{video.captions && <track kind="captions" src={video.captions} srcLang="en" label="English" />}</video><div className="mission-video-list" aria-label="Choose an animated guide">{videos.map((item) => <button type="button" key={item.id} aria-pressed={item.id === video.id} onClick={() => setActiveVideo(item.id)}><Play aria-hidden /><span><strong>{item.title}</strong><small>{item.length}</small></span></button>)}</div></div></section>}
