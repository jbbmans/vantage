...
import { ArrowLeft, Copy, FileText, Award, GraduationCap, Target, ShieldAlert, UserX, UserCheck, KeyRound, LogOut } from 'lucide-react';
import * as apiClient from '@/lib/api';
import { unitPath, displayName, refreshAll } from '@/store/useStore';
...
export default function MemberDetail() {
  ...
  // Finding 27: the access review is the authority check — the server only
  // answers it for someone entitled to administer this Marine, so its very
  // presence is what unlocks the account controls below.
  ...
  const resetPw = () => act(() => apiClient.resetMemberPassword(id, newPw), () => {
    ...
    toast.success('Password reset. Every session for this Marine was signed out.');
    loadReview();
  });
    ...
    <div className="mx-auto max-w-[1200px] space-y-3">
      ...
      {review && (
        <Panel
          ...
          action={review.user.is_admin ? <Badge tone="signal">Administrator</Badge> : null}
        >
          {review.findings?.length > 0 && (
      ...
      {pwDialog && (
        <Dialog
          ...
          description="Every session they hold will be signed out. Hand them the new password directly."
          footer={
            <>
              <Button variant="ghost" size="sm" onClick={() => { setPwDialog(false); setNewPw(''); }}>Cancel</Button>
              <Button size="sm" disabled={busy || newPw.length < 10} onClick={resetPw}>Reset password</Button>
            </>
          }
        >
          <Field label="New password" hint="At least 10 characters">
            <Input type="text" autoComplete="off" value={newPw} onChange={(e) => setNewPw(e.target.value)} />
          </Field>
        </Dialog>
      )}
    </div>
