...
function Appearance() {
  ...
  return (
    <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
      <Panel title="Theme" className="lg:col-span-2">
        <div className="flex flex-wrap items-end gap-x-10 gap-y-5">
          <div><p className="mb-2 text-xs font-semibold text-ink-2">Mode</p><Segmented label="Theme" value={theme} onChange={(v) => save.mutate({ theme: v })} options={[{ value: 'light', label: <span className="flex items-center gap-1.5"><Sun className="h-4 w-4" />Light</span> }, { value: 'dark', label: <span className="flex items-center gap-1.5"><Moon className="h-4 w-4" />Dark</span> }, { value: 'system', label: <span className="flex items-center gap-1.5"><Monitor className="h-4 w-4" />System</span> }]} /></div>
          <div><p className="mb-2 text-xs font-semibold text-ink-2">Density</p><Segmented label="Density" value={prefs.density || 'comfortable'} onChange={(v) => save.mutate({ density: v })} options={[{ value: 'comfortable', label: 'Comfortable' }, { value: 'compact', label: 'Compact' }]} /></div>
        </div>
      </Panel>
      <Panel title="Color" subtitle="A palette for the whole app: the page, cards, navigation, tabs and buttons, tuned for light and dark." className="lg:col-span-2">
        <div className="grid grid-cols-1 gap-2 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">{ACCENTS.map((a) => {
