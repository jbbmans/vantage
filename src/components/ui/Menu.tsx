...
export function MenuContent({ children, align = 'end', className }: { children: React.ReactNode; align?: 'start' | 'end' | 'center'; className?: string }) {
  return (
    <DropdownMenu.Portal>
      <DropdownMenu.Content align={align} sideOffset={6} className={cn('z-50 min-w-[220px] rounded-xl bg-surface p-1.5 shadow-pop animate-scale-in', className)}>{children}</DropdownMenu.Content>
    </DropdownMenu.Portal>
  );
}
export function MenuItem({ children, onSelect, danger, icon: Icon, disabled }: { children: React.ReactNode; onSelect?: () => void; danger?: boolean; icon?: React.ComponentType<{ className?: string }>; disabled?: boolean }) {
  return (
    <DropdownMenu.Item disabled={disabled} onSelect={onSelect} className={cn('flex cursor-pointer select-none items-center gap-2.5 rounded-lg px-2.5 py-[7px] text-sm outline-none transition-colors data-[highlighted]:bg-surface-2 data-[disabled]:opacity-40', danger ? 'text-bad' : 'text-ink-2 data-[highlighted]:text-ink')}>
      {Icon && <Icon className="h-4 w-4 opacity-80" />}{children}
    </DropdownMenu.Item>
  );
}
