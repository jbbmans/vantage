    ...
    <Ctx.Provider value={api}>
      ...
      <div className="pointer-events-none fixed inset-x-0 bottom-[max(1rem,env(safe-area-inset-bottom))] z-[60] flex flex-col items-center gap-2 px-4 sm:items-end sm:pr-6" aria-live="polite">
        ...
        {toasts.map((t) => {
          ...
          return (
            // A confirmation rises into place, and when one goes the other slides to fill the gap,
            // so two in a row read as two, not as one that flickered.
            <m.div key={t.id} role={t.kind === 'error' ? 'alert' : 'status'} layout
              ...
              className={cn('pointer-events-auto flex w-full max-w-sm items-start gap-2.5 rounded-lg border bg-surface px-3.5 py-2.5 text-sm shadow-pop', t.kind === 'error' ? 'border-bad/40' : t.kind === 'success' ? 'border-good/40' : 'border-line')}>
