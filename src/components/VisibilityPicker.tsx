...
export default function VisibilityPicker({ value, unitId, onChange, compact = false }: { value: 'private' | 'unit'; unitId?: string | null; onChange: (v: { visibility: 'private' | 'unit'; unit_id?: string | null }) => void; compact?: boolean }) {
  ...
  const options: Array<{ value: 'private' | 'unit'; label: string; icon: typeof Lock; hint: string; disabled?: boolean }> = [
    ...
    { value: 'unit', label: 'Share with unit', icon: Users, hint: canShare ? 'Readable by leaders with the View shared records permission.' : 'Join a unit to share.', disabled: !canShare },
  ];
  return (
    <div className="min-w-0">
      ...
      <div className={cn('grid gap-2', compact ? 'grid-cols-2' : 'sm:grid-cols-2')} role="radiogroup" aria-label="Visibility">
        {options.map((o) => (
          <button key={o.value} type="button" role="radio" aria-checked={value === o.value} disabled={o.disabled} onClick={() => onChange({ visibility: o.value, unit_id: o.value === 'unit' ? unitId || identity?.primaryUnitId || null : unitId })}
