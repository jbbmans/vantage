...
/**
 * An attribute value is always written inside double quotes, but it may have been read from single
 * quotes or none, so it can carry a `"` that would end the attribute early and start a new one (a
 * `style` that covers the page, say). Quotes are encoded as well as the text escapes.
 */
