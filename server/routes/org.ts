...
orgRouter.delete('/units/:unitId/members/:userId', wrap((req, res) => {
  ...
  audit(ctx, { actor_id: req.user.id, action: 'remove_member', entity: 'unit', entity_id: unitId, subject_id: userId, unit_id: unitId, detail: `roles: ${removed.roles}; records frozen: ${removed.recordsFrozen}; claims released: ${removed.claimsReleased}; sessions revoked: ${revoked}`, ip: clientIp(req) });
