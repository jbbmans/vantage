...
- **Work**: the queue of project items (open work, open to claim, mine), projects, tasks, and correspondence. Each item has its own page with an append-only history. Every claim, handoff, stage, wait, observation, decision, submission, funds check and verification is attributed and dated.
...
- **Records of every kind.** Work, education, certifications, training and PME, volunteering, extracurriculars, physical fitness and awards. Each kind asks its own questions (credits and grade, issuer and expiry, hours and role) and saves exactly those; only work carries money. Filter by period, category, area, and quality (missing outcome, untagged, duplicates). Edit, attach evidence files, restore from a 30-day recycle bin.
...
- **Team, for everyone on it.** Every member sees their team's roster, access levels and where its work stands, and every team in the organization is listed by name. Team leaders also see who carries what and open members' shared records, every open logged. Unit dashboard built from shared entries only, roles with per-unit permissions, invitations by link or email, access log.
- **People and access levels.** Three levels, held per team: *Personal*, *Team leader* and *Administrator*. People (`/people`) sets them, adds and removes team members, and invites at a chosen level; an organization administrator also suspends, restores and resets sign-in for any account. Nobody changes their own access, every change signs the person out and is audited. See `docs/engineering/ENTERPRISE_READINESS.md`.
...
- **Operations.** `GET /api/health/live` and `/api/health/ready` for an orchestrator; `npm run backup` for a scheduled, integrity-checked, fingerprinted backup that prunes old copies. See `docs/operations.md`.
